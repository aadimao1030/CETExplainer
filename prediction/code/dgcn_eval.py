...
for fold in folds:
    ...
    x_all = pd.read_csv(f"../data/node_representation/x_all_{emb_dim}_epoch5000.csv", header=0).values
    ...
    model.load_weights(os.path.join('../data', 'weights', f'mode{mode}_fold{fold}_epoch{epoch}_learnRate{lr}_batchsize{bs}_embdim{emb_dim}_layer2.h5'))

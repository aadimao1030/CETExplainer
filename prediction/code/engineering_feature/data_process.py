...
def process(pub_smiles, mutation_feature, gexpr_feature, copy_number_feature, data_new, nb_celllines, nb_drugs):
    ...
    for pubchem_id, isosmiles in zip(pub_smiles['pubchems'], pub_smiles['isosmiles']):
        ...
        drug_feature[str(pubchem_id)] = [X[0].get_atom_features(), X[0].get_adjacency_list(), 1]
    ...
    copy_number_feature=copy_number_feature.loc[cellid]
    ...
    copy_number = torch.from_numpy(np.array(copy_number_feature,dtype='float32'))
    ...
    cellline_set = Data.DataLoader(dataset=Data.TensorDataset(mutation,gexpr,copy_number),batch_size=nb_celllines,shuffle=False)

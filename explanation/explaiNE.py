...
if __name__ == '__main__':
    ...
    for fold in range(0, 5):
        ...
        model.load_weights(os.path.join(f'../prediction/data/weights/mode0_fold{fold}_epoch5000_learnRate0.001_batchsize256_embdim64.h5'))
        ...
        for head, rel, tail in tqdm(tf_data.map(lambda x,y,z: (tf.cast(x, tf.float32), tf.cast(y, tf.float32), tf.cast(z, tf.float32))), total=len(tf_data)):
        ...
        np.savez(f'data/explaiNE_preds_fold{fold}_layer2_Epoch5000.npz', preds=preds)

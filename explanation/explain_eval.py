...
explain_id = 2
fold = 2
gt_fold = np.load(f'data/gt_filtered_fold2.npz', allow_pickle=True)['result']
if explain_id == 1:
    # 2 is used for CETExplainer, 1 for GNNExplainer and 0 for explaiNE
    explaine_preds = np.load(f'data/GNNExplainer_preds_fold2_layer2_epoch5000.npz', allow_pickle=True)['preds']
elif explain_id == 0:
    explaine_preds = np.load(f'data/explaiNE_preds_fold2_layer2.npz', allow_pickle=True)['preds']
elif explain_id == 2:
    explaine_preds = np.load(f'data/CETExplainer_preds_fold2_layer2_Epoch5000_exEpoch70.npz', allow_pickle=True)['preds']
